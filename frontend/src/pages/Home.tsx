...
import { MdAutoFixHigh, MdWorkspacePremium, MdStar, MdChatBubble, MdPhotoCamera, MdFolderSpecial } from 'react-icons/md';
...
export default function Home() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-3 sm:px-4 py-6 sm:py-12 text-[#FDFBF7] overflow-x-hidden">
      ...
      {/* GŁÓWNE FOLDERY (KATEGORIE) */}
      <section className="mb-6 sm:mb-8">
        <div className="flex flex-wrap items-center gap-2 mb-3">
            <MdFolderSpecial className="text-[#D4AF37] text-lg sm:text-xl" />
            <span className="text-[10px] sm:text-xs font-black text-gray-400 uppercase tracking-wider">Katalogi przepisów:</span>
        </div>
        <div className="flex flex-wrap justify-start sm:justify-start gap-2">
          <button
            onClick={() => ustawParametr('category', null)}
            className={`inline-block px-4 py-2 sm:px-6 sm:py-3 rounded-xl shadow-sm font-black text-xs sm:text-sm uppercase transition-all duration-300 flex-shrink-0 ${
      ...
      {/* CHMURA DODATKOWYCH TAGÓW (Filtry dynamiczne) */}
      {tags.length > 0 && (
        <section className="mb-6 bg-[#0D1321] p-3 sm:p-4 rounded-2xl border border-[#540B0E]">
          <div className="flex flex-wrap items-center gap-2 w-full">
            <span className="text-[10px] sm:text-xs font-black text-gray-400 uppercase tracking-wider mr-1 sm:mr-2">Dodatkowe tagi:</span>
